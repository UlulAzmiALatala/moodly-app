...
// --- Komponen Ikon (Sama persis dengan Customer) ---
...
export default function CounselorHelpPage() {
    const navigate = useNavigate();

    // Data FAQ Khusus Konselor (Konten Beda, Tampilan Sama)
    const faqs = [
        "Bagaimana cara mengatur jadwal praktik?",
        "Kapan gaji saya akan dicairkan?",
        "Apa yang harus dilakukan jika pasien tidak hadir?",
        "Bagaimana cara mengubah profil dan keahlian?",
        "Apa sanksi jika membatalkan sesi mendadak?",
    ];

    // Navigasi ke halaman detail FAQ Konselor
    const handleSelectFAQ = (faqItem) => {
        navigate("/counselor/help/faq", { state: { question: faqItem } });
    };

    // Navigasi ke halaman Chat Admin Konselor
    const handleContactNow = () => {
        navigate("/counselor/help/chat-admin");
    };

    return (
        // Container utama (Visual sama persis dengan Customer)
        <div className="flex justify-center min-h-screen bg-white">
            <div className="w-full max-w-md min-h-screen bg-white flex flex-col shadow-lg relative">
                {/* === HEADER === */}
                <header className="relative h-80 flex-shrink-0">
                    {/* Background biru melengkung (Sama persis) */}
                    ...
                    <div className="relative z-10 flex justify-center items-start h-full pt-12">
                        <img
                            ...
                            onError={(e) => {
                                ...
                                e.target.src =
                                    "https://via.placeholder.com/192x192/00A9E0/FFFFFF?text=Help";
                            }}
                        />
                    </div>
                </header>
                ...
                {/* === MAIN CONTENT === */}
                <main className="flex-grow flex flex-col justify-center p-8 bg-white">
                    <div className="w-full">
                        <h1 className="text-2xl font-bold text-center text-gray-800 mb-2">
                            Pusat Bantuan Mitra
                        </h1>
                        <p className="text-sm text-gray-500 text-center mb-6">
                            Panduan untuk Konselor Moodly
                        </p>
                        ...
                        {/* Tombol Hubungi Sekarang (Hitam, sama seperti Customer) */}
                        <button
