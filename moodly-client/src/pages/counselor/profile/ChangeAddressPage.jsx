...
export default function CounselorChangeAddressPage() {
    const navigate = useNavigate();
    const location = useLocation();
    const { user, getUser } = useAuth();

    // Menentukan mode tampilan: 'full' (Lokasi) atau 'street' (Jalan)
    ...
    const handleSubmit = async (e) => {
        ...
        // Filter data yang akan dikirim berdasarkan mode
        let dataToSubmit = {};
        if (mode === "street") {
            dataToSubmit = { street_address: formData.street_address };
        } else {
            // Mode 'location' atau 'full' mengirim data lokasi geografis
            dataToSubmit = {
                province: formData.province,
                city: formData.city,
                district: formData.district,
                postal_code: formData.postal_code,
                // Jika mode full, kirim semua. Jika hanya lokasi, jalan tidak wajib di sini tapi backend mungkin butuh.
                // Untuk aman, kita kirim apa yang ada di form yang tampil, tapi API update biasanya merge data.
                // Di sini kita kirim semua formData untuk amannya, atau spesifik.
                // Sesuai request: "Lokasi" hanya prov, kota, kec, pos.
            };
            if (mode === "full") {
        ...
        try {
            // API Endpoint khusus Konselor
            await apiClient.post("/api/counselor/profile/update", dataToSubmit);

            await getUser(); // Refresh data user global
            setSuccessMessage("Alamat berhasil diperbarui!");

            setTimeout(() => {
                navigate("/counselor/profile/edit"); // Kembali ke halaman edit profil konselor
            }, 1500);
        } catch (err) {
        ...
        <div className="bg-white min-h-full font-sans">
            ...
            <main className="p-5 pt-6">
                <form className="space-y-5" onSubmit={handleSubmit}>
                    ...
                    {/* Mode JALAN (Hanya Alamat Lengkap) */}
                    {(mode === "street" || mode === "full") && (
                        <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                            <TextAreaField
                                ...
                                placeholder="Masukkan detail nama jalan, nomor rumah, RT/RW..."
                            />
                        </div>
                    )}
